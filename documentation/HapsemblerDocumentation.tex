...
\texttt{hapsemble} is a wrapper script that executes the programs in the core Hapsembler pipeline in a logical order. Note that this script will not work unless the full path of the Hapsembler bin directory is added to your system path. The synopsis of the script and the basic options are described below.
...
Above, the orientation of the reads are expected to be forward-reverse. If you have more than one library, you can use the -f and -i options repeatedly. For example, if you have two libraries in files lib1.fastq and lib2.fastq with insert sizes 300bp and 600bp, respectively:
...
where $C$ is a small (usually less than 1GB) overhead. Note that, the above should still be taken as lower bounds: the actual memory used by these programs might vary in different environments.
...
Though the memory imprint of \texttt{scarpa} is not very high, its running time partially depends on the quality of the contigs and reads.
