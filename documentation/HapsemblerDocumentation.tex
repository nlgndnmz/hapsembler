...
Suppose you have $n$ reads totalling to $M$ base pairs, $t$ threads are used and let $k=14$ if the genome size to be assembled is less than 270mbp, and $k=15$ otherwise. Then the approximate memory requirement for \texttt{encore} is:
...
We note that the actual requirements may vary with the environment. The speed-up gained from multiple-threads is almost linear in the number of threads used as long as the machine is not overcrowded. Thus, we strongly recommend enabling multi-threading if you have sufficient memory available. 
